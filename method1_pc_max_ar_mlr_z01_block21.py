...
# method1_pc_max_ar_mlr_z01_block21.py

#

# 방법1 (Zhang et al. 2024 영감):

#   벌금률 정의를 PC = rate · max(DA, RT)로 변경

#   이익함수: DA*x + RP*y+ - PC*y-  (3항)

#   oracle: 3후보 {0, actual, 1.0}, PC = rate*max(DA,RT)
...
# 3. Gap 계산 — 3항, PC = rate * max(DA, RT), oracle 3후보

# =====================================================================

def compute_gap(pc_rate, pred, actual, da, rt):

    """이익함수: DA*x + RP*y+ - PC*y-, PC = rate*max(DA,RT)"""
...
# 6. AR 제안모형 MILP — PC = rate*max(DA, RT)
...
# 7. MLR 제안모형 MILP — PC = rate*max(DA, RT)
...
def solve_mlr(pc_rate, W1, W2):
    ...
    for i in range(n_obs):
        ...
        pc = pc_rate * max(mlr_train_da[i], mlr_train_rt[i])
...
gc = os.path.join(OUT_DIR, "grid_method1_pc_max_ar_mlr_z01_block21.csv")
...
print(f"""
  ...
  │  AR  (z01/블럭21) │ {ar_kn:>10.2f}   │ {ar_kg:>10.2f}   │ {ar_nrmse:>10.2f}   │ {ar_bg:>10.2f}   │

  │  MLR (z01/블럭21) │ {mlr_kn:>10.2f}   │ {mlr_kg:>10.2f}   │ {mlr_nrmse:>10.2f}   │ {mlr_bg:>10.2f}   │
...
save_csv(os.path.join(OUT_DIR, "fig3_method1_z01.csv"),

         ["Label","nRMSE","Gap"],

         [[FIG_LABELS[i+1], round(f3n[i],2), round(f3g[i],2)] for i in range(10)])

save_csv(os.path.join(OUT_DIR, "fig5_method1_z01.csv"),
...
save_csv(os.path.join(OUT_DIR, "fig6_method1_z01.csv"),

         ["Label","nRMSE","Gap"],

         [[FIG_LABELS[i+1], round(f6n[i],2), round(f6g[i],2)] for i in range(10)])

save_csv(os.path.join(OUT_DIR, "fig8_method1_z01.csv"),
...
fig.suptitle(f"Fig.3 — 방법1(PC=rate·max(DA,RT)), AR, rate={FIG36_RATE}", fontsize=13)
...
p3 = os.path.join(RESULTS_DIR, "fig3_method1_pc_max_ar_z01.png")
...
f5.suptitle(f"Fig.5 — 방법1, AR, W1=W2=1, rate 스윕", fontsize=13)
...
p5 = os.path.join(RESULTS_DIR, "fig5_method1_pc_max_ar_z01.png")
...
fig6.suptitle(f"Fig.6 — 방법1(PC=rate·max(DA,RT)), MLR, rate={FIG36_RATE}", fontsize=13)
...
p6 = os.path.join(RESULTS_DIR, "fig6_method1_pc_max_mlr_z01.png")
...
f8.suptitle(f"Fig.8 — 방법1, MLR, W1=W2=1, rate 스윕", fontsize=13)
...
p8 = os.path.join(RESULTS_DIR, "fig8_method1_pc_max_mlr_z01.png")
...
print("  완료 — 방법1 PC=max(DA,RT) AR+MLR 통합 (z01/블럭21)")
