...
TRAIN_START = pd.Timestamp(os.environ.get("TRAIN_START", "2013-08-25"))   # 학습 시작일 [블록7]
TRAIN_END = pd.Timestamp(os.environ.get("TRAIN_END",   "2013-11-22"))     # 학습 마지막일 [블록7]
TEST_START = pd.Timestamp(os.environ.get("TEST_START",  "2013-11-23"))    # 테스트 시작일 [블록7]
TEST_END = pd.Timestamp(os.environ.get("TEST_END",    "2013-12-22"))      # 테스트 마지막일 [블록7]
...
ZHANG_C = float(os.environ.get("ZHANG_C", "1.5"))   # [Zhang et al. 방식] 부족가격 rho_plus = ZHANG_C * RT (c=1.5가 논문 재현 최적값)
...
for hour in range(HOURS_PER_DAY):                       # 시간대 0부터 11까지 순서대로 하나씩 처리
    ...
    gmodel = gp.Model(f"baseline_ar_hour_{hour}")            # Gurobi 모델 생성
    gmodel.Params.OutputFlag = 0                               # 콘솔 로그 끔

    beta_var = gmodel.addMVar(n_features, lb=-GRB.INFINITY, name="beta")   # 회귀계수(부호 제한 없음)
    u_var = gmodel.addMVar(n_ar_rows, lb=0.0, name="u")                     # 절대오차 보조변수

    fitted_expr = ar_design_matrix @ beta_var                                # 예측값 (선형식)
    ...
    gmodel.setObjective(u_var.sum() / n_ar_rows, GRB.MINIMIZE)               # 평균절대오차(LAD) 최소화
    gmodel.optimize()

    coefficients_by_hour[hour] = beta_var.X                    # 결과에서 beta(13개)를 이 시간대의 계수로 저장
    lp_success = (gmodel.Status == GRB.OPTIMAL)

    print(f"  시간대 {hour} 회귀 완료 (성공 여부: {lp_success})")   # 진행상황 출력
...
for i in range(len(actual_flat)):          # 테스트 1200개 관측치를 하나씩 순서대로 처리
    ...
    penalty_cost_i = ZHANG_C * rt_i     # [Zhang et al.] 부족가격 rho_plus = c*RT (기존: PENALTY_RATE*DA)
