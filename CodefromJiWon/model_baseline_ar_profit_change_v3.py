...
# =====================================================================
# model_baseline_ar_profit_change_v3.py
#
# [재검토 후 수정] AR 기준선만으로는 Fig.3(W1/W2에 따른 dual축 곡선)을
# 그릴 수 없다고 했었는데, 이 파일 안에서 제안 모형(MILP, 4항 이익함수)
# W1/W2 스윕까지 직접 풀면 얼마든지 그릴 수 있다 - model_proposed_ar_
# profit_change_v3.py의 solve_proposed_milp() 로직을 그대로 가져와 통합.
#
#   - Fig.3: 벌금율 50% 고정, W1/W2 10개점(AR 포함 11개) 스윕 → dual y축
#            (model_proposed_ar_profit_change_v3_fig3.png와 동일한 형태)
#   - Fig.5: W1=W2=1 고정, 벌금율 0~100% 스윕 → nRMSE/Gap 좌우 2분할
#            (AR + 제안모형 둘 다 표시)
#   - 4항 이익함수(부족분 = 벌금(rate*DA) + RT가격)를 양쪽 다 사용.
#
# 산출물: results/model_baseline_ar_profit_change_v3_fig3.csv, _fig5.csv,
#         results/model_baseline_ar_profit_change_v3_fig3.png, _fig5.png
#
# 코딩 스타일: model_baseline_ar_profit_change.py 와 마찬가지로 class는
#             쓰지 않는다. 같은 MILP를 (rate, W1, W2) 조합만큼 반복해야
#             하므로, 그 부분만 함수로 묶는다.
# =====================================================================
...
FIG3_PAPER_AR_NRMSE = 34.76         # 논문 Table 3, 기본 모형 AR 의 nRMSE(%) - 비교용
FIG3_PAPER_AR_GAP = 15.04           # 논문 Table 3, 기본 모형 AR 의 optimality gap(%) - 비교용
...
#    - 이번엔 학습 구간도 DA/RT 가격이 같이 필요함 (제안 모형은 학습 때
#      가격도 손실함수에 쓰기 때문)
# =====================================================================
...
# =====================================================================
# 5. AR 학습용 입력(X), 정답(y) 만들기 - 논문 Eq.(3): "직전 하루"의
#    12시간을 입력으로 씀
# =====================================================================
...
#    - 4항: profit = DA*x + RT*surplus - (PC+RT)*shortage, PC=rate*DA
#    - 오라클: {0, 실제발전량, 설비최대(1.0)} 3후보
# =====================================================================
def compute_nrmse(pred_flat, actual_flat):
    rmse_value = np.sqrt(np.mean((actual_flat - pred_flat) ** 2))
    return 100.0 * rmse_value / np.mean(actual_flat)


def compute_gap(pred_flat, penalty_rate, actual_flat, da_flat, rt_flat):
    sum_realized = 0.0
    sum_oracle = 0.0
...
ar_nrmse = compute_nrmse(ar_pred_flat, actual_flat)   # 벌금율과 무관 - 한 번만 계산


# =====================================================================
# 8. (penalty_rate, W1, W2) 하나를 받아 제안모형 MILP를 풀고 계수를 반환
#    하는 함수 - model_proposed_ar_profit_change_v3.py 8절 로직 그대로.
#    4항: shortage_cost = W1*(penalty+RT) + W2 (부족분은 벌금+RT가격)
# =====================================================================
...
# =====================================================================
# 9. 캐시가 달린 조회 함수 - (rate, W1, W2) 조합을 이미 풀었으면 재사용
#    (Fig.3의 W1/W2=1/1@rate0.5 와 Fig.5의 W1/W2=1/1 스윕이 겹칠 수 있음)
# =====================================================================
...
def get_proposed_result(penalty_rate, W1, W2):
    ...
    if key not in solved_cache:
        coeffs = solve_proposed_milp(penalty_rate, W1, W2)
        pred_flat = predict_test(coeffs)
        nrmse = compute_nrmse(pred_flat, actual_flat)
        gap = compute_gap(pred_flat, penalty_rate, actual_flat, da_flat, rt_flat)
        solved_cache[key] = (nrmse, gap)
    return solved_cache[key]


# =====================================================================
# 10. Fig.3 데이터 생성 — 벌금율 50% 고정, W1/W2 10개점(AR 포함 11개) 스윕
# =====================================================================
print("\n" + "=" * 60)
print(f"  Fig.3 데이터 생성 (벌금율={FIG3_PENALTY_RATE} 고정, 4항 이익함수)")
print("=" * 60)

fig3_ar_gap = compute_gap(ar_pred_flat, FIG3_PENALTY_RATE, actual_flat, da_flat, rt_flat)
...
for idx, (W1, W2) in enumerate(FIG3_W_RATIOS):
    nrmse, gap = get_proposed_result(FIG3_PENALTY_RATE, W1, W2)
...
# =====================================================================
# 11. Fig.5 데이터 생성 — W1=W2=1 고정, 벌금율 0~100% 스윕
# =====================================================================
print("\n" + "=" * 60)
...
for rate in FIG5_PENALTY_RATES:
    ar_gap_r = compute_gap(ar_pred_flat, rate, actual_flat, da_flat, rt_flat)
...
fig3_csv_path = os.path.join(out_dir, "model_baseline_ar_profit_change_v3_fig3.csv")
...
fig5_csv_path = os.path.join(out_dir, "model_baseline_ar_profit_change_v3_fig5.csv")
...
# =====================================================================
# 13. Fig.3 그리기 — 하나의 그래프에 nRMSE(왼쪽 y축) + Gap(오른쪽 y축)
#     (model_proposed_ar_profit_change_v3_fig3.png 와 동일한 dual y축 구성)
# =====================================================================
...
fig3_png_path = os.path.join(out_dir, "model_baseline_ar_profit_change_v3_fig3.png")
figure3.savefig(fig3_png_path, dpi=150)
print(f"\nsaved: {fig3_png_path}")


# =====================================================================
# 14. Fig.5 그리기 — nRMSE / Gap 을 좌우 두 그래프로 나눠서 (AR + 제안모형)
# =====================================================================
...
fig5_png_path = os.path.join(out_dir, "model_baseline_ar_profit_change_v3_fig5.png")
