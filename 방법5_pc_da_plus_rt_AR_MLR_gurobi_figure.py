# -*- coding: utf-8 -*-
# =====================================================================
# 방법5_pc_da_plus_rt_AR_MLR_gurobi_figure.py
#
# `방법5_pc_da_plus_rt_AR_MLR_gurobi.py`(검증 완료된 Gurobi 변환본)를 그대로
# 복사한 뒤, savefig/CSV 저장 경로에만 "_gurobi" 접미사를 붙인 버전이다.
# Figure 생성 코드 자체는 새로 짠 것이 아니라 원본 `방법5_pc_da_plus_rt_AR_
# MLR.py`(scipy) 안에 원래 있던 코드를 그대로 물려받은 것이다 — 이 파일이
# 곧 results/방법5_fig{3,5,6,8}_..._z03_block18.png를 실제로 만드는 스크립트였다
...
#
# ── Gurobi 변환 메모 ──
# 이 파일은 `방법5_pc_da_plus_rt_AR_MLR.py`의 scipy.optimize.milp/linprog(HiGHS)
...
RATES    = [round(0.1 * i, 1) for i in range(11)]  # 0~100% (방법2/4와 동일하게 확장)

# ── W2 균형 보정 Weight — 방법4와 같은 이유로 여기서도 적용하지 않는다 ──
# (raw 계수 구조라 W1 항이 W2보다 훨씬 크지만, 방법4에서 Weight=4를 포팅했을 때 오히려
# collapse 구간이 넓어지는 걸 확인했다. PC=rate*(DA+RT)도 raw 계수라 같은 위험이 있어
# 기본값 1(무보정)로 둔다.)
W2_BALANCE_WEIGHT = 1
...
# 제안모형 W1/W2 스윕 논문값 (Table 3/4) — 이 스크립트는 1/20~1/1 5개점만 다루므로 앞 5개만 사용
PAPER_AR_PROP_NRMSE_FULL = [34.89, 35.14, 36.28, 41.09, 44.95,
                            46.11, 48.27, 49.21, 49.61, 50.07]
PAPER_AR_PROP_GAP_FULL   = [13.91, 13.42, 12.71, 11.88, 11.44,
                            11.38, 11.38, 11.36, 11.36, 11.36]
PAPER_MLR_PROP_NRMSE_FULL = [21.92, 21.84, 21.75, 21.66, 22.01,
                             23.32, 27.75, 30.62, 35.76, 37.67]
PAPER_MLR_PROP_GAP_FULL   = [11.91, 11.68, 11.15, 10.65, 10.28,
                             9.91, 9.51, 9.32, 9.27, 9.28]
PAPER_AR_PROP_NRMSE  = PAPER_AR_PROP_NRMSE_FULL[:len(W_LABELS)]
PAPER_AR_PROP_GAP    = PAPER_AR_PROP_GAP_FULL[:len(W_LABELS)]
PAPER_MLR_PROP_NRMSE = PAPER_MLR_PROP_NRMSE_FULL[:len(W_LABELS)]
PAPER_MLR_PROP_GAP   = PAPER_MLR_PROP_GAP_FULL[:len(W_LABELS)]
...
# 방법2(PC=RT+rate*DA)에서는 oracle을 3후보{0,actual,1.0}→2후보{0,actual}로 줄여도 됐지만
# (§8.1, baseline Gap이 논문과 ±0.1%p 일치), 이 formula(PC=rate*(DA+RT))도 rate=0일 때
# PC=0이 되어 방법4와 같은 이유로 2후보로 줄이면 Gap이 음수가 될 수 있다. 3후보를 유지한다.
...
def solve_ar(pc_rate, W1, W2):
    ...
    for hour in range(HOURS_PER_DAY):
        ...
        for i in range(n_obs):
            ...
            sc[i] = (-W1 * rt_h[i]) + W2_eff    # 논문 Eq.(9a), 정규화 제거
            yc[i] = (W1 * pc) + W2_eff          # 논문 Eq.(9a), 정규화 제거
...
# =====================================================================
# 9. Fig5/8 데이터 — rate 0~100% (W1=W2=1)
# =====================================================================
print("\n" + "=" * 70)
print("  5. Fig.5/8 데이터 — rate 0~100% (W1=W2=1, raw 계수 그대로 — 방법2식 W2 보정 미적용)")
print("=" * 70)
...
save_csv(os.path.join(OUT_DIR, "fig3_pc_da_plus_rt_AR_gurobi.csv"), ["Label","nRMSE","Gap"],
         [[W_LABELS[i], round(f3n[i],2), round(f3g[i],2)] for i in range(len(W_LABELS))])
save_csv(os.path.join(OUT_DIR, "fig5_pc_da_plus_rt_AR_gurobi.csv"), ["Rate","nRMSE","Gap"],
         [[RATES[i], round(f5_pn[i],2), round(f5_pg[i],2)] for i in range(len(RATES))])
save_csv(os.path.join(OUT_DIR, "fig6_pc_da_plus_rt_MLR_gurobi.csv"), ["Label","nRMSE","Gap"],
         [[W_LABELS[i], round(f6n[i],2), round(f6g[i],2)] for i in range(len(W_LABELS))])
save_csv(os.path.join(OUT_DIR, "fig8_pc_da_plus_rt_MLR_gurobi.csv"), ["Rate","nRMSE","Gap"],
...
x_w = list(range(len(W_LABELS_ALL)))
...
fig.suptitle("방법5_fig3 — AR PC=rate×(DA+RT), W1/W2 스윕 (rate=0.5)", fontsize=13)
ax_r = ax.twinx()
...
fig.tight_layout()
p3 = os.path.join(RESULTS_DIR, "방법5_fig3_ar_z03_block18_gurobi.png")
...
# RATES는 이제 0~100%(11개점)로 논문 Fig.5 판독값과 인덱스가 그대로 일치
rate_idx = [round(r*10) for r in RATES]
fig5_paper_ar_n = [FIG5_PAPER_AR_NRMSE[i] for i in rate_idx]
fig5_paper_prop_n = [FIG5_PAPER_PROP_NRMSE[i] for i in rate_idx]
fig5_paper_ar_g = [FIG5_PAPER_AR_GAP[i] for i in rate_idx]
fig5_paper_prop_g = [FIG5_PAPER_PROP_GAP[i] for i in rate_idx]
fig5, (a5n, a5g) = plt.subplots(1, 2, figsize=(13, 5))
fig5.suptitle("방법5_fig5 — AR PC=rate×(DA+RT), W1=W2=1, penalty rate 스윕", fontsize=13)

a5n.plot(x_rates, fig5_paper_ar_n, ls="--", color=C_AR, marker="o", markersize=4,
...
a5n.set_ylim(*auto_ylim(fig5_paper_ar_n, fig5_paper_prop_n, [ar_nrmse], f5_pn))
...
a5g.set_ylim(*auto_ylim(fig5_paper_ar_g, fig5_paper_prop_g, f5_bg, f5_pg))

fig5.tight_layout()
p5 = os.path.join(RESULTS_DIR, "방법5_fig5_ar_z03_block18_gurobi.png")
...
fig6.suptitle("방법5_fig6 — MLR PC=rate×(DA+RT), W1/W2 스윕 (rate=0.5)", fontsize=13)
ax6_r = ax6.twinx()
...
fig6.tight_layout()
p6 = os.path.join(RESULTS_DIR, "방법5_fig6_mlr_z03_block18_gurobi.png")
...
fig8_paper_mlr_n = [FIG8_PAPER_MLR_NRMSE[i] for i in rate_idx]
fig8_paper_prop_n = [FIG8_PAPER_PROP_NRMSE[i] for i in rate_idx]
fig8_paper_mlr_g = [FIG8_PAPER_MLR_GAP[i] for i in rate_idx]
fig8_paper_prop_g = [FIG8_PAPER_PROP_GAP[i] for i in rate_idx]
fig8, (a8n, a8g) = plt.subplots(1, 2, figsize=(13, 5))
fig8.suptitle("방법5_fig8 — MLR PC=rate×(DA+RT), W1=W2=1, penalty rate 스윕", fontsize=13)

a8n.plot(x_rates, fig8_paper_mlr_n, ls="--", color=C_MLR, marker="o", markersize=4,
...
a8n.set_ylim(*auto_ylim(fig8_paper_mlr_n, fig8_paper_prop_n, [mlr_nrmse], f8_pn))
...
a8g.set_ylim(*auto_ylim(fig8_paper_mlr_g, fig8_paper_prop_g, f8_bg, f8_pg))

fig8.tight_layout()
p8 = os.path.join(RESULTS_DIR, "방법5_fig8_mlr_z03_block18_gurobi.png")
