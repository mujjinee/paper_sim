...
identity_matrix = sparse.eye(n_train_obs, format="csr")      # 1080x1080 단위행렬 (절대값 처리용 보조변수 계수)
...
for i in range(n_test_obs):                # 테스트 1200개 관측치를 하나씩 순서대로 처리
    ...
    surplus_if_full = max(actual_i - 1.0, 0.0)                                      # [B-2] 약정 1.0일 때 잉여
    shortage_if_full = max(1.0 - actual_i, 0.0)                                     # [B-2] 약정 1.0일 때 부족량
    profit_if_commit_full = CAPACITY_MW * DURATION_HOURS * (
        da_i * 1.0 + rt_i * surplus_if_full - penalty_cost_i * shortage_if_full
    )                                                                                # [B-2] 약정=설비최대(1.0)일 때 이익
    oracle_profit_i = max(profit_if_commit_zero, profit_if_commit_actual, profit_if_commit_full)   # 셋 중 더 큰 쪽
