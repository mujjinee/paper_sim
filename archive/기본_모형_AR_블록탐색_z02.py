...
# 기본_모형_AR_블록탐색.py
#
# 목적: 새로 확장된 merged_for_simulation_z01.csv (2012-04-01~2014-07-01,
#       predictors15.csv 전체 구간을 거의 다 커버) 전체를 400일 블록
#       (학습 300일 + 테스트 100일)으로 30일씩 겹치게 슬라이딩하면서,
#       "기본 모형 AR"(bounded LAD, 논문 Eq.3)의 nRMSE가 구간마다 어떻게
#       달라지는지 확인한다. 이전에 했던 블록9는 이 중 한 위치일 뿐이고,
#       이번엔 이제 데이터가 더 커진 만큼 처음부터 끝까지 다시 훑는다.
#
# 코딩 스타일: class, def(함수) 를 전혀 쓰지 않는다. 위에서 아래로
#             순서대로 실행되는 코드만 쓴다(naive 스타일). 거의 모든
#             줄에 그 줄이 뭘 하는지 주석을 단다.
#
# 데이터: merged_for_simulation_z02.csv (Zone2)
...
MERGED_FILE = os.path.join(BASE_DIR, "merged_for_simulation_z02.csv")  # 읽어올 병합 데이터 파일 경로 (Zone 2)
...
STEP_DAYS = 30                   # 다음 블록으로 넘어갈 때 며칠씩 옮길지 (400일보다 작아서 블록끼리 겹침)
...
# =====================================================================
# 3. 400일 블록을 30일씩 겹치게 슬라이딩하면서, 블록마다 기본 모형 AR 을
#    학습(bounded LAD, 논문 Eq.3 "직전 하루" 입력) + 예측 + 채점
# =====================================================================

n_blocks = (n_total_days - WINDOW_DAYS) // STEP_DAYS + 1     # 만들 수 있는 400일 블록 개수
...
for block_number in range(n_blocks):                            # 블록 0번째부터 마지막 블록까지 순서대로
    ...
    train_solar = window_solar[:TRAIN_DAYS]                          # 앞쪽 300일 = 학습용
    test_solar = window_solar[TRAIN_DAYS:]                            # 뒤쪽 100일 = 테스트용
    ...
    if window_start > 0:                                              # 블록 시작 전에 날짜가 있으면(첫 블록이 아니면)
        ...
        ar_target_solar = train_solar                                    # 학습 목표값은 300일 전부 사용
    ...
    # ---- 3-4. 테스트 100일을 하루씩 순서대로 예측 (rolling one-day-ahead) ----
...
best_block = min(block_results, key=lambda row: row[3])                # nRMSE 가 가장 작은(=가장 좋은) 블록
worst_block = max(block_results, key=lambda row: row[3])               # nRMSE 가 가장 큰(=가장 나쁜) 블록

print()
print(f"최고 블록: 블록{best_block[0]} (테스트 {best_block[1]}~{best_block[2]}) nRMSE={best_block[3]:.2f}%")
print(f"최저 블록: 블록{worst_block[0]} (테스트 {worst_block[1]}~{worst_block[2]}) nRMSE={worst_block[3]:.2f}%")
