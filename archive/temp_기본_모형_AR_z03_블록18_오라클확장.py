...
for i in range(len(actual_flat)):          # 테스트 1200개 관측치를 하나씩 순서대로 처리
    ...
    # ---- [B-2 실험] 오라클(사후 최적) 이익 계산: {0, 실제발전량, 설비최대(1.0)} 3후보 중 더 큰 쪽
    #      (기존은 {0,S} 2후보뿐이라 "약정>S"가 더 이득인 경우를 못 봤음 - 28번 문서 B-2) ----
    ...
    surplus_if_full = max(actual_i - 1.0, 0.0)                     # 약정 1.0일 때 잉여(보통 0)
    ...
    )                                                                # ↑ 약정을 설비최대(1.0)로 하면의 이익

    oracle_profit_i = max(profit_if_commit_zero, profit_if_commit_actual, profit_if_commit_full)   # 셋 중 더 이익나는 쪽 선택
