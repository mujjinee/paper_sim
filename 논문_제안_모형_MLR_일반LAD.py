...
# =====================================================================
# 논문_제안_모형_MLR_일반LAD.py
#
# 목적: 논문_제안_모형_MLR.py 는 fitted commitment(x_t = Ŝ_t)에 [0,1]
#       상한을 걸어뒀다(bounds 의 upper_bounds=ones(...)). 이번엔 그
#       상한을 x, y'(surplus), y''(shortage) 세 변수에서만 제거해서
#       ("일반 LAD" 와 대응되는 버전) 다시 풀어본다.
#
#       주의(이론적 위험, 사전 검증 완료): 복잡성(complementarity) 제약만
#       있고 x 의 상한이 없으면, 한계효용
#         dZ/dx = W1*scale*(PC_t-DP_t)/denominator + W2/n
#       이 음수인 행이 있으면 x_t -> 무한대로 가야 목적값이 계속
#       줄어드는 "unbounded" 문제가 생길 수 있다. 이 스크립트를 만들기
#       전에 학습 3600행 전부에 대해 이 값을 미리 계산해봤고, 전부
#       양수(0.00508~0.00550)라 이번 데이터/W1=1,W2=20 조합에서는
#       실제로 unbounded 가 되지는 않는다는 걸 먼저 확인한 뒤 실행한다.
#       (z, 즉 상보조건용 binary 는 원래대로 [0,1]로 유지 - 정의상 binary라
#        상한을 없애면 안 됨)
#
# 목적(원문): "논문 제안 모형 MLR" (논문 Eq.10 - 발전계획 최적화를 MLR 학습에
...
BOUNDED_NRMSE = 32.174187           # 논문_제안_모형_MLR.py (x,y',y'' 상한=1 유지) 결과 - 비교용
BOUNDED_GAP = 2.586135              # 논문_제안_모형_MLR.py (x,y',y'' 상한=1 유지) 결과 - 비교용
...
#      (bounded 버전과의 차이는 여기뿐: x, y_plus, y_minus 의 "상한=1" 을 뺐다.
#       z 는 정의상 binary 라 [0,1] 상한을 그대로 유지해야 함)
lower_bounds = np.concatenate([
    np.full(n_features, -np.inf),          # beta: 아래 제한 없음
    np.zeros(3 * n_train_obs + n_binary),    # x, y_plus, y_minus, z: 0 이상 (동일)
])
upper_bounds = np.concatenate([
    np.full(n_features, np.inf),            # beta: 위 제한 없음
    np.full(3 * n_train_obs, np.inf),         # x, y_plus, y_minus: 상한 없음  <- bounded 버전과 다른 부분
    np.ones(n_binary),                         # z 만 [0,1] 유지 (binary 라서)
])
...
print()
print("=== 논문 제안 모형 MLR - 일반 LAD (x,y',y'' 상한 없음) - 블록9 결과 ===")
print(f"nRMSE          = {nrmse_percent:.6f} %   (논문: {PAPER_NRMSE:.2f} %, bounded: {BOUNDED_NRMSE:.6f} %)")
print(f"optimality gap = {optimality_gap_percent:.6f} %   (논문: {PAPER_GAP:.2f} %, bounded: {BOUNDED_GAP:.6f} %)")
print()
bounded_label = "bounded (x,y+,y- 상한=1, 논문_제안_모형_MLR.py)"       # 비교용 행 이름
general_label = "일반 LAD (상한 없음, 이 파일)"                          # 이번 결과 행 이름
print(f"{'구성':<45}{'nRMSE':>10}{'gap':>10}")
print(f"{'논문 (참고)':<45}{PAPER_NRMSE:>9.2f}%{PAPER_GAP:>9.2f}%")
print(f"{bounded_label:<45}{BOUNDED_NRMSE:>9.4f}%{BOUNDED_GAP:>9.4f}%")
print(f"{general_label:<45}{nrmse_percent:>9.4f}%{optimality_gap_percent:>9.4f}%")
