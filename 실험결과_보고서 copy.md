...
**방법 6(SPO+)은 접근 자체가 다르다** — 벌금가·이익식은 건드리지 않고 학습 목적함수를 SPO+ loss로 바꿨다. Gap의 부호(음수→양수)는 고쳐졌지만 nRMSE는 여섯 방법 중 가장 크게 벌어져서, 근본 문제(DA−PC>0)를 없애지는 못했다 — 다만 방법2(PC=RT+rate×DA)와 결합하면(방법6+2) 어느 정도 보완된다(5.6절 참고).
...
#### 3.2.6 방법 6 — Smart SPO+ loss

방법 1~5는 전부 **이익식(profit formula)** 자체를 고치는 접근이었다. 방법 6은 반대로 이익식은 방법 2(3.2.2절, PC=RT+rate×DA)를 그대로 두고, **학습 목적함수**를 W1/W2 혼합 대신 SPO+ loss로 바꿨다 — "예측이 얼마나 틀렸는가"가 아니라 "그 예측으로 내린 약정 결정이 최선의 결정(oracle)보다 얼마나 손해를 봤는가"(**후회, regret**)를 직접 줄이는 방식이다.
...
| 방법 2 (PC=RT+rate×DA) | 35.69% | 14.54% | nRMSE +0.8%p, Gap +0.6%p — 가장 가까움 |
...
> 방법 4는 이 rate=0.5 지점에서만 보면 안 좋아 보이지만, rate를 0.9~1.2쪽으로 옮기면 훨씬 나아진다 — 5.4절에서 그래프로 보여준다. 방법 6의 수치는 z03/블록18 스크립트 기준이며, z01/블록21 교차검증은 5.6절 참고.
...
| 2 (PC=RT+rate×DA) | `integrated_4term_ar_mlr_z03_block18.py` | z03/블록18 | 35.69% | 14.54% |
...
| 6+2 (SPO+ × 방법2 결합) | `integrated_spo_plus_4term_fig3568_AR_MLR.py` | z01/블록21 | 48.93% | 16.23% |
| 논문(Table 3, 참고) | — | — | 34.89% | 13.91% |
...
스크립트가 다르다 보니 위 표와 완전히 같은 숫자는 아니다 — 특히 **방법1은 오라클·제약 처리 디테일 차이로 Gap 부호까지 달라졌다**(위 표 +9.6%p vs 여기 -1.09%p). 하지만 전체적인 순위는 그대로 재확인됐다: **방법1~6을 한 틀에서 다시 비교해도 방법2(PC=RT+rate×DA)와 Zhang 비대칭(방법3)이 논문에 가장 가깝고**(nRMSE +0.8%p 안팎), **PC=max(DA,RT)(방법4)가 nRMSE·Gap 둘 다 가장 크게 벌어졌다**(+8.8%p, +8.1%p). 방법5(PC=DA+RT)는 nRMSE는 3위권이지만 Gap이 논문보다 오히려 3.6%p 작게 나왔다. 방법6+2(SPO+와 방법2를 합친 조합, z01/블록21 데이터라 직접 비교는 조심스럽다)는 nRMSE가 가장 크게 벌어졌는데, SPO+ loss 자체의 특성(5.6절)과 z01이라는 다른 블록의 조합 효과가 섞여 있어 보인다.
...
| 방법 1 (논문 수식) | 24.25% | 3.02% | nRMSE +2.3%p, Gap −8.9%p — Gap이 크게 작음 |
| 방법 2 (PC=RT+rate×DA) | 24.07% | 11.49% | nRMSE +2.2%p, Gap +0.4%p — 가장 가까움 |
...
| 방법 6+2 (SPO+×방법2, z01/블록21) | 43.03% | 14.78% | nRMSE +21.1%p, Gap +2.9%p — z01 데이터라 직접 비교는 참고용 |

**AR·MLR을 같이 놓고 보면**: 방법2·3(PC=RT+rate×DA·Zhang)이 AR·MLR 둘 다 Gap 오차 1%p 이내로 가장 안정적이고, 방법4(PC=max)는 AR·MLR 둘 다 가장 크게 벌어진다는 게 일관되게 재확인된다. 다만 nRMSE는 MLR 쪽이 방법에 관계없이 전반적으로 논문보다 +2%p 안팎 높게 나오는 경향이 있다 — 외생 변수(dSSRD, dTSR)를 학습에 쓰는 MLR 특성상 90일(z03/블록18) 학습 데이터로는 논문(정확한 기간 미공개)만큼 변수 관계를 못 잡는 것으로 보인다.
...
|  |  |
|---|---|
| ![방법1 Fig.3](results/fig3_paper_W1W2_AR.png) | ![방법1 Fig.5](results/fig5_paper_rate_AR.png) |
| 방법1 Fig.3 — AR, W1/W2 스윕 (dual y축) | 방법1 Fig.5 — AR, 벌금율 스윕 |
...
방법 2는 방법 1의 벌금가(PC)를 $RT + rate \cdot DA$로 재정의해, 3항 구조를 유지하면서 부족분의 실시간시장 매입비용을 반영한 것이다(수식은 3.2.2절, 논문 근거는 첨부 1 참고). W1/W2를 1/20부터 1/0까지 쭉 스윕해도 논문 곡선처럼 nRMSE는 완만히 오르고 Gap은 완만히 내려가는 모양이 그대로 나왔다. AR 기준선 Gap도 14.94%로 논문(15.04%)과 거의 맞았다.
...
- **Fig.3(AR, c=1.5, W1/W2 스윕)**: 포화 없이 36%에서 69% 선까지 완만하게 상승하고, gap은 14.5%에서 11% 선까지 내려간 뒤 2/1 부근에서 살짝 반등해 11~11.5% 사이에서 유지된다. 전체적인 모양은 방법2(PC=RT+rate×DA)와 거의 비슷하다.
...
아래는 통합 스크립트(`integrated_pc_da_plus_rt_fig3568_AR_MLR.py`, z03/블록18, rate=50% 고정)로 W1/W2·penalty rate 전 구간을 그린 것이다.
...
1/20~1/1 구간에서 36.09~37.98%/10.46~10.70%로 매끄럽게 움직였고, 2/1 이후에도 79.62%→100.29%로 계속 값이 바뀌면서(=완전히 평평해지지 않음) **방법1과 달리 고-W1 구간에서도 극단 포화가 나타나지 않았다.** 이유는 3.2.4절에서 설명한 방법4의 한계를 뒤집으면 된다 — 방법5의 PC=rate·(DA+RT)는 RT가 충분히 크면 DA를 넘어설 수 있어서, 방법4(PC=rate·max(DA,RT))이 갖고 있던 "RT<DA 구간에서는 PC<DA"라는 구멍이 메워지기 때문이다.
...
같은 SPO+ 방식을 다른 블록(z01/블록21)에서도 따로 돌려서 교차검증했다 — 아래는 그 결과다(위 z03 수치와는 다른 스크립트·데이터라 숫자가 정확히 같지는 않다). W1 비중이 커질수록 nRMSE·Gap이 67~116%/13~16% 선까지 완만히 벌어지는, 방법1보다는 온건한 곡선이다.
...
위는 SPO+ loss만 단독으로 적용한 버전이고, 아래는 여기에 방법2(PC=RT+rate×DA)까지 같이 결합한 버전(`integrated_spo_plus_4term_fig3568_AR_MLR.py`, z01/블록21)이다.

|  |  |
|---|---|
| ![방법6+2 Fig.3](results/fig3_spo_plus_4term_W1W2_AR.png) | ![방법6+2 Fig.5](results/fig5_spo_plus_4term_rate_AR.png) |
| 방법6+2 Fig.3 — SPO+×방법2, AR, W1/W2 스윕 (rate=50%) | 방법6+2 Fig.5 — SPO+×방법2, AR, penalty rate 스윕 (W1=W2=1) |

|  |  |
|---|---|
| ![방법6+2 Fig.6](results/fig6_spo_plus_4term_W1W2_MLR.png) | ![방법6+2 Fig.8](results/fig8_spo_plus_4term_rate_MLR.png) |
| 방법6+2 Fig.6 — SPO+×방법2, MLR, W1/W2 스윕 (rate=50%) | 방법6+2 Fig.8 — SPO+×방법2, MLR, penalty rate 스윕 (W1=W2=1) |
...
| 6위 (참고) | 방법 6+2 (SPO+×방법2) | 31.0%p | 24.0%p | nRMSE 이탈이 가장 크지만 z01/블록21 데이터라 다른 방법(z03/블록18)과 직접 비교는 어렵다 — 4장의 계절별 분석대로 z01/블록21은 원래 baseline 난이도 자체가 높은 블록이다. |
