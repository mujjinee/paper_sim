...
이고, $k=4$(즉 실질 W2=80)가 논문 곡선에 가장 가까웠다 — 그래서 방법 8은 "W1=1, W2=20의 4배" 지점으로 정의한다. 부족분 평가(Gap 계산용 오라클)는 방법 2의 §5.2.1(초기)과 동일하게 commit=0/commit=실제/commit=1.0 3후보를 쓴다(2.1절 표준, §5.8) — 한때 2후보로 줄였던 적이 있었으나("PC=RT+rate×DA는 rate=0이어도 PC=RT>0이라 commit=1.0이 오라클이 될 수 없다"는 근거였다), rate=0이면 잉여/부족 구간 기울기가 둘 다 DA−RT로 같아져 꺾이는 점이 없어지고, DA>RT인 시간대(z03/블록18 테스트셋의 62.8%)에서는 commit=1.0이 실제 오라클이라는 게 확인돼(부록 `방법1_방법6.1_이익식_비교분석.md` 참고) 3후보로 되돌렸다. 다행히 이 문제는 rate가 낮은 구간(Fig.5/8)에서만 Gap을 최대 +7%p 과소평가했을 뿐, KPI 지점(rate=0.5)에서는 영향이 0에 가까워(재계산해도 소수점까지 동일) 3.3절 수치·"4배가 최적"이라는 결론 모두 바뀌지 않았다.
...
배수 $k$를 1배~15배까지 스윕하며 논문 Fig.5 판독값(rate 0/10/30/50/70/100%) 대비 SSE(nRMSE+Gap 오차 제곱합)를 비교한 결과(오라클 3후보 기준 — 아래 참고):
...
부족분 Gap 평가는 오라클을 commit=0/commit=실제/commit=1.0 3후보를 쓴다(2.1절 표준). 한때 commit=0/commit=실제 2후보로 줄였던 적이 있었으나("PC=RT+rate×DA는 rate=0이어도 PC=RT>0이라 commit=1.0이 오라클이 될 수 없다"는 근거였다), 이 근거가 틀렸다는 게 확인돼(rate=0이면 잉여/부족 구간 기울기가 둘 다 DA−RT로 같아져 commit=1.0이 실제 오라클이 되는 시간대가 z03/블록18 테스트셋의 62.8%나 됨 — 부록 `방법1_방법6.1_이익식_비교분석.md` 참고) 3후보로 되돌렸다(2026-09-14). rate=0에서 Gap이 6.48%→13.53%로 크게 바뀌었지만, rate≥0.5에서는 영향이 거의 0이라 위 SSE 표의 "4배가 최적"이라는 결론과 3.3절 KPI 수치는 바뀌지 않았다 — 위 표의 SSE 값 자체만 이전(2후보 기준: 4467/941/380/369/431/568/983/1070/1134/1220)보다 소폭 상향 조정됐다.
...
| 🥈 2위 | <span style="color:red; text-decoration:line-through">방법 2 (Fine Tune, §5.2.2)</span> <span style="color:blue">방법 8 (PC=RT+rate×DA, W2=20의 4배, §5.8절)</span> | 2.1%p | 2.6%p | <span style="color:red; text-decoration:line-through">**22.5%p** (Fig.3 5.4+Fig.5 7.3+Fig.6 4.7+Fig.8 5.2)</span> <span style="color:blue">**23.6%p** (Fig.3 5.4+Fig.5 7.5+Fig.6 4.7+Fig.8 6.0 — 2026-09-14 오라클 3후보 수정 후 재계산. Fig.3·Fig.6은 rate=0.5 고정이라 오라클 수정 영향이 없어 그대로, Fig.5·Fig.8은 rate 낮은 구간 왜곡이 고쳐져 소폭 상승)</span> | <span style="color:red; text-decoration:line-through">raw 계수+Weight=4로 재보정한 방법2다.</span> <span style="color:blue">방법2와 이익식은 같고, 기준 W2=20의 4배 지점을 찾아 쓴 것이다(3.2.8절).</span> 전 지점으로 보면 1위 방법6(21.0%p)과 거의 대등하다 — KPI 1점 기준 차이(0.3%p vs 2.1%p)만큼 크게 벌어지지는 않는다. |
