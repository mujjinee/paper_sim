...
# paper_eq_ar_mlr_z03_block18.py

#

# 논문 원식 (Zhang et al. 2024, Eq.1a/9/13 그대로):

#   벌금률(penalty cost) PC = rate · DA  (부족분 벌금단가, 일간전 가격의 rate%)

#   이익함수: DA*x + RP*y+ - PC*y-  (3항)

#   학습(제안모형 MILP W1/W2 정규화용) oracle: 3후보 {0, actual, 1.0}, PC = rate*DA

#   평가(최종 nRMSE/Gap 계산용, Eq.13) oracle: 2후보 {0, actual} — 설비최대 후보 없음
...
PAPER_AR_NRMSE  = 34.76   # 기본 모형 AR (baseline)

PAPER_AR_GAP    = 15.04

PAPER_MLR_NRMSE = 21.76   # 기본 모형 MLR (baseline)

PAPER_MLR_GAP   = 12.59

PAPER_AR_PROP_NRMSE  = 34.89   # 논문 제안 모형 AR (W1=1, W2=20, rate=50%)

PAPER_AR_PROP_GAP    = 13.91

PAPER_MLR_PROP_NRMSE = 21.92   # 논문 제안 모형 MLR (W1=1, W2=20, rate=50%)

PAPER_MLR_PROP_GAP   = 11.91
...
# 3. Gap 계산 — 논문 Eq.1a/13 그대로: 3항, PC = rate * DA, oracle 2후보 {0, actual}

# =====================================================================

def compute_gap(pc_rate, pred, actual, da, rt):

    """이익함수: DA*x + RP*y+ - PC*y-, PC = rate*DA. 평가 oracle은 {0, actual} 2후보(Eq.13)."""
...
# 6. AR 제안모형 MILP (논문 Eq.9) — PC = rate*DA
...
def solve_ar(pc_rate, W1, W2):
    ...
    for hour in range(HOURS_PER_DAY):
        ...
        # 학습용(W1/W2 정규화 상수) oracle: 3후보 {0, actual, 1.0}, PC=rate*DA
...
# 7. MLR 제안모형 MILP (논문 Eq.9) — PC = rate*DA
...
gc = os.path.join(OUT_DIR, "grid_paper_eq_ar_mlr_z03_block18.csv")
...
print(f"""
  ...
  ※ 논문 Table 3/4 기준값 (z03/블록18) 대비:

      AR  baseline : nRMSE={PAPER_AR_NRMSE}%, Gap={PAPER_AR_GAP}%   (이번 구현: {ar_nrmse:.2f}%, {ar_bg:.2f}%)

      AR  제안모형  : nRMSE={PAPER_AR_PROP_NRMSE}%, Gap={PAPER_AR_PROP_GAP}%   (이번 구현: {ar_kn:.2f}%, {ar_kg:.2f}%)

      MLR baseline : nRMSE={PAPER_MLR_NRMSE}%, Gap={PAPER_MLR_GAP}%   (이번 구현: {mlr_nrmse:.2f}%, {mlr_bg:.2f}%)

      MLR 제안모형  : nRMSE={PAPER_MLR_PROP_NRMSE}%, Gap={PAPER_MLR_PROP_GAP}%   (이번 구현: {mlr_kn:.2f}%, {mlr_kg:.2f}%)
...
save_csv(os.path.join(OUT_DIR, "fig3_paper_eq_z03.csv"),

         ["Label","nRMSE","Gap"],

         [[FIG_LABELS[i+1], round(f3n[i],2), round(f3g[i],2)] for i in range(10)])

save_csv(os.path.join(OUT_DIR, "fig5_paper_eq_z03.csv"),
...
save_csv(os.path.join(OUT_DIR, "fig6_paper_eq_z03.csv"),

         ["Label","nRMSE","Gap"],

         [[FIG_LABELS[i+1], round(f6n[i],2), round(f6g[i],2)] for i in range(10)])

save_csv(os.path.join(OUT_DIR, "fig8_paper_eq_z03.csv"),
...
fig.suptitle(f"Fig.3 — 논문방법(PC=rate·DA), AR, rate={FIG36_RATE}", fontsize=13)
...
p3 = os.path.join(RESULTS_DIR, "fig3_paper_eq_ar_z03.png")
...
f5.suptitle(f"Fig.5 — 논문방법, AR, W1=W2=1, rate 스윕", fontsize=13)
...
p5 = os.path.join(RESULTS_DIR, "fig5_paper_eq_ar_z03.png")
...
fig6.suptitle(f"Fig.6 — 논문방법(PC=rate·DA), MLR, rate={FIG36_RATE}", fontsize=13)
...
p6 = os.path.join(RESULTS_DIR, "fig6_paper_eq_mlr_z03.png")
...
f8.suptitle(f"Fig.8 — 논문방법, MLR, W1=W2=1, rate 스윕", fontsize=13)
...
p8 = os.path.join(RESULTS_DIR, "fig8_paper_eq_mlr_z03.png")
...
print("  완료 — 논문방법(PC=rate·DA) AR+MLR 통합 (z03/블록18)")
