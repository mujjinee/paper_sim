# -*- coding: utf-8 -*-
# =====================================================================
# 방법6_SPO_base_AR_MLR.py
#
# SPO+ 1차 시뮬레이션(base) — PC=rate*DA (RT 재정의 이전의 원래 4항식 벌금)
# + Elmachtoub & Grigas(2022)식 SPO+ shortage-cost 조건부 가중치.
# 원본: spo_plus_ar_mlr_z01_block21.py (2026-09-09) 를 방법2/4/5/6과 같은
# 컨벤션(raw 계수+W2 balance weight, 제목/파일명, auto_ylim, Fig.8 논문값)으로
# 재정리한 버전 — 5.6절 "1차 시뮬레이션"에 해당.
#
# 이익식(4항, PC=rate*DA):
#   profit = scale × (DA·x + RP·surplus − RP·shortage − PC·shortage)
#
# SPO+ shortage cost의 조건부 가중치(원본 spo_plus_ar_mlr_z01_block21.py 특징):
#   PC는 DA>RT일 때만 shortage 비용에 추가된다 — I[DA>RT]
#   (DA>RT면 shortage 시 RT+PC 모두 손실, RT≥DA면 RT만 손실)
#
# raw 계수 + W2 balance weight(방법2/6과 동일한 이유로 적용):
...
# ── W2 균형 보정 Weight — 방법6(SPO+ 2차)과 동일한 값으로 시작해서 비교한다 ──
W2_BALANCE_WEIGHT = 4
...
# =====================================================================
# 6. SPO+ base MILP — AR (시간대별, raw 계수 + W2 balance weight)
#
# shortage cost: W1·RT + W1·PC·I[DA>RT] + W2_eff   (PC=rate*DA, indicator 조건부)
# surplus  cost: −W1·RT + W2_eff
# =====================================================================
def solve_ar_spo_base(penalty_rate, W1, W2):
    """AR SPO+ base: PC=rate*DA(I[DA>RT] 조건부), raw 계수 + W2 balance weight"""
    W2_eff = W2 * W2_BALANCE_WEIGHT
    ...
    for hour in range(HOURS_PER_DAY):
        ...
        yc = W1 * rt_h + W1 * pc_h * indicator + W2_eff  # shortage cost (SPO+ 조건부)
...
# =====================================================================
# 7. SPO+ base MILP — MLR (pooled, raw 계수 + W2 balance weight)
# =====================================================================
def solve_mlr_spo_base(penalty_rate, W1, W2):
    """MLR SPO+ base: PC=rate*DA(I[DA>RT] 조건부), raw 계수 + W2 balance weight"""
    W2_eff = W2 * W2_BALANCE_WEIGHT
    n_obs = n_train_obs
    ...
    r = milp(c=obj, integrality=integ, bounds=Bounds(lb, ub),
             constraints=all_con, options={"mip_rel_gap": 1e-9})
    if not r.success:
        raise RuntimeError(f"MLR SPO+ base MILP 실패: {r.message}")
...
def get_ar(pr, w1, w2):
    ...
    if k not in cache_ar:
        pred = solve_ar_spo_base(pr, w1, w2)
...
def get_mlr(pr, w1, w2):
    ...
    if k not in cache_mlr:
        pred = solve_mlr_spo_base(pr, w1, w2)
...
# =====================================================================
print("\n" + "=" * 70)
print("  4. 그리드 스윕 (rate 10개 x W1/W2 5개 = 50조합, SPO+ base AR+MLR)")
print("=" * 70)
...
grid_csv = os.path.join(OUT_DIR, "grid_spo_base_ar_mlr_z03_block18.csv")
...
save_csv(os.path.join(OUT_DIR, "fig3_spo_base_AR.csv"),
         ["Label", "nRMSE", "Gap"],
         [[FIG_LABELS[i], round(fig3_ar_n[i], 2), round(fig3_ar_g[i], 2)]
          for i in range(len(FIG_LABELS))])

save_csv(os.path.join(OUT_DIR, "fig5_spo_base_AR.csv"),
...
save_csv(os.path.join(OUT_DIR, "fig6_spo_base_MLR.csv"),
         ["Label", "nRMSE", "Gap"],
         [[FIG_LABELS[i], round(fig6_mlr_n[i], 2), round(fig6_mlr_g[i], 2)]
          for i in range(len(FIG_LABELS))])

save_csv(os.path.join(OUT_DIR, "fig8_spo_base_MLR.csv"),
...
fig.suptitle(f"방법6_fig3_base — AR SPO+base PC=rate×DA(I[DA>RT]), rate={FIG36_RATE}, W1/W2 스윕", fontsize=13)
ax_r = ax_l.twinx()
...
fig.tight_layout()
p3 = os.path.join(RESULTS_DIR, "방법6_fig3_base_ar_z03_block18.png")
...
fig5_fig.suptitle(f"방법6_fig5_base — AR SPO+base PC=rate×DA(I[DA>RT]), W1=W2=1, penalty rate 스윕", fontsize=13)
...
p5 = os.path.join(RESULTS_DIR, "방법6_fig5_base_ar_z03_block18.png")
...
fig6_fig.suptitle(f"방법6_fig6_base — MLR SPO+base PC=rate×DA(I[DA>RT]), rate={FIG36_RATE}, W1/W2 스윕", fontsize=13)
ax_r = ax_l.twinx()
...
p6 = os.path.join(RESULTS_DIR, "방법6_fig6_base_mlr_z03_block18.png")
...
fig8_fig.suptitle(f"방법6_fig8_base — MLR SPO+base PC=rate×DA(I[DA>RT]), W1=W2=1, penalty rate 스윕", fontsize=13)
...
p8 = os.path.join(RESULTS_DIR, "방법6_fig8_base_mlr_z03_block18.png")
...
# =====================================================================
print("\n" + "=" * 70)
print("  완료 — SPO+ base PC=rate×DA(I[DA>RT]) 통합 실험 (AR+MLR, z03/블록18)")
print("=" * 70)
print(f"""
  생성 파일:
    Fig.3 (AR W1/W2):   {p3}
  ...
  ※ SPO+ base = 1차 시뮬레이션 (PC=rate×DA, I[DA>RT] 조건부 shortage cost)
     2차(방법6_SPO_plus_AR_MLR.py)는 여기에 PC=RT+rate×DA 재정의를 적용한 버전
""")
