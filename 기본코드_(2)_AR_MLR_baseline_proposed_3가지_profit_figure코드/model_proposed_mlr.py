...
for i in range(n_train_obs):                                    # 3600개를 하나씩 순서대로
    ...
    shortage_cost[i] = (W1 * penalty_i) + W2           # [수정] 정규화 전부 제거 - 논문 Eq.(10a) 문자 그대로
...
#     nRMSE·gap 표로 뽑는다 (원본 3항 profit function 버전)
# =====================================================================
...
paper_nrmse_list = [21.76, 21.92, 21.84, 21.75, 21.66, 22.01, 23.32, 27.75, 30.62, 35.76, 37.67]
paper_gap_list = [12.59, 11.91, 11.68, 11.15, 10.65, 10.28, 9.91, 9.51, 9.32, 9.27, 9.28]

fig, ax1 = plt.subplots(figsize=(9, 5))
ax2 = ax1.twinx()

ax1.plot(chart_labels, sweep_nrmse, color="#F97316", marker="o", label="nRMSE (우리 결과)")
ax1.plot(chart_labels, paper_nrmse_list, color="#FDBA74", marker="o", linestyle="--", label="nRMSE (논문 Table4)")
ax2.plot(chart_labels, sweep_gap, color="#3B82F6", marker="s", label="gap (우리 결과)")
ax2.plot(chart_labels, paper_gap_list, color="#93C5FD", marker="s", linestyle="--", label="gap (논문 Table4)")
...
plt.title("Fig.6 원본(3항, 정규화X) - z03 블록18")
plt.tight_layout()
plt.savefig("fig6_mlr_original.png", dpi=150)
print("저장 완료: fig6_mlr_original.png")
plt.show()
...
for penalty_rate_sw in PENALTY_LIST:
    ...
    for i in range(n_train_obs):
        ...
        shortage_cost_p8[i] = (FIG8_W1 * penalty_i) + FIG8_W2
    ...
    print(f"[벌금율 {penalty_rate_sw*100:.0f}%] MLR nRMSE={mlr_nrmse_f8:.4f}% gap={gap_mlr_sw:.4f}% | Proposed nRMSE={nrmse_p8:.4f}% gap={gap_p8:.4f}%")

paper_mlr_nrmse_list = [21.76] * 11
paper_mlr_gap_list = [8.2, 9.5, 10.5, 11.5, 12.3, 12.59, 14.0, 15.0, 15.8, 16.5, 17.0]
paper_proposed_nrmse_list = [24.0, 22.3, 21.8, 21.4, 21.8, 22.01, 22.6, 23.0, 23.7, 25.0, 26.1]
paper_proposed_gap_list = [7.5, 8.7, 9.3, 9.6, 10.0, 10.28, 10.6, 10.8, 11.0, 11.0, 11.0]

penalty_pct_labels = [f"{int(p*100)}%" for p in PENALTY_LIST]

fig, (ax_a, ax_b) = plt.subplots(1, 2, figsize=(12, 5))
ax_a.plot(penalty_pct_labels, mlr_nrmse_list, color="#3B82F6", marker="o", label="MLR (우리 결과)")
ax_a.plot(penalty_pct_labels, paper_mlr_nrmse_list, color="#93C5FD", linestyle="--", label="MLR (논문 Fig.8 추정치)")
ax_a.plot(penalty_pct_labels, proposed_nrmse_list, color="#F97316", marker="s", label="Proposed (우리 결과)")
ax_a.plot(penalty_pct_labels, paper_proposed_nrmse_list, color="#FDBA74", linestyle="--", label="Proposed (논문 Fig.8 추정치)")
ax_a.set_xlabel("Penalty Cost Rate")
ax_a.set_ylabel("nRMSE (%)")
ax_a.set_title("(a) nRMSE")
ax_a.legend(fontsize=8)

ax_b.plot(penalty_pct_labels, mlr_gap_list, color="#3B82F6", marker="o", label="MLR (우리 결과)")
ax_b.plot(penalty_pct_labels, paper_mlr_gap_list, color="#93C5FD", linestyle="--", label="MLR (논문 Fig.8 추정치)")
ax_b.plot(penalty_pct_labels, proposed_gap_list, color="#F97316", marker="s", label="Proposed (우리 결과)")
ax_b.plot(penalty_pct_labels, paper_proposed_gap_list, color="#FDBA74", linestyle="--", label="Proposed (논문 Fig.8 추정치)")
ax_b.set_xlabel("Penalty Cost Rate")
ax_b.set_ylabel("Optimality Gap (%)")
ax_b.set_title("(b) Optimality Gap")
ax_b.legend(fontsize=8)

plt.suptitle("Fig.8 원본(3항, 정규화X) - z03 블록18 (W1=W2=1)")
plt.tight_layout()
plt.savefig("fig8_mlr_original.png", dpi=150)
print("저장 완료: fig8_mlr_original.png")
plt.show()