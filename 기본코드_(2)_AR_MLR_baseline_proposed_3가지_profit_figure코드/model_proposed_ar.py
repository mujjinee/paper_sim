...
# =====================================================================
# 10. Table 3 스윕 - 0/1(기존 재현) + W1/W2 비율 10개 (원본 3항)
# =====================================================================
...
for ratio_label, sweep_w1, sweep_w2 in RATIO_LIST:
    ...
    coefficients_by_hour_sw = np.zeros((HOURS_PER_DAY, n_features))
    ...
    for hour in range(HOURS_PER_DAY):
        ...
        for i in range(n_train_obs):
            ...
            surplus_cost_sw[i] = (-sweep_w1 * rt_this_hour[i]) + sweep_w2
            shortage_cost_sw[i] = (sweep_w1 * penalty_i) + sweep_w2
        ...
        gmodel_sw = gp.Model(f"proposed_ar_sweep_{ratio_label}_{hour}")
        ...
        gmodel_sw.addConstr(x_var_sw - ar_design_matrix @ beta_var_sw == 0.0)
        gmodel_sw.addConstr(x_var_sw + yplus_var_sw - yminus_var_sw == y_this_hour)
        ...
        objective_expr_sw = (
            (-sweep_w1 * da_this_hour) @ x_var_sw
        ...
        coefficients_by_hour_sw[hour] = beta_var_sw.X

    test_forecast_sw = np.zeros((n_test_days, HOURS_PER_DAY))
    previous_day_actual_sw = train_solar[-1]

    for day_index in range(n_test_days):
        feature_vector_sw = np.concatenate([[1.0], previous_day_actual_sw[::-1]])
        for hour in range(HOURS_PER_DAY):
            raw_prediction = np.dot(coefficients_by_hour_sw[hour], feature_vector_sw)
            test_forecast_sw[day_index, hour] = min(max(raw_prediction, 0.0), 1.0)
        previous_day_actual_sw = test_solar[day_index]

    actual_flat_sw = test_solar.flatten()
    predicted_flat_sw = test_forecast_sw.flatten()

    sse_sw = 0.0
    for i in range(len(actual_flat_sw)):
        e = actual_flat_sw[i] - predicted_flat_sw[i]
        sse_sw = sse_sw + e * e
    rmse_sw = (sse_sw / len(actual_flat_sw)) ** 0.5
    avg_actual_sw = 0.0
    for i in range(len(actual_flat_sw)):
        avg_actual_sw = avg_actual_sw + actual_flat_sw[i]
    avg_actual_sw = avg_actual_sw / len(actual_flat_sw)
    nrmse_sw = 100.0 * rmse_sw / avg_actual_sw

    da_flat_sw = test_da_price.flatten()
    rt_flat_sw = test_rt_price.flatten()

    sum_realized_sw = 0.0
    sum_oracle_sw = 0.0
    for i in range(len(actual_flat_sw)):
        actual_i = actual_flat_sw[i]
        commitment_i = predicted_flat_sw[i]
        da_i = da_flat_sw[i]
        rt_i = rt_flat_sw[i]
    ...
    print(f"[{ratio_label:>8}] nRMSE={nrmse_sw:.4f}%  gap={gap_sw:.4f}%")

print()
print("| W1/W2 | nRMSE | optimality gap |")
print("|---|---:|---:|")
for lbl, nr, gp_ in zip(sweep_labels, sweep_nrmse, sweep_gap):
    print(f"| {lbl} | {nr:.4f}% | {gp_:.4f}% |")
    

# =====================================================================
# 11. Figure 3 재현 - nRMSE(왼쪽 축)·optimality gap(오른쪽 축)을
#     W1/W2 비율별로 그린다 (원본 3항 profit function 버전)
# =====================================================================
import matplotlib.pyplot as plt
plt.rcParams['font.family'] = 'Malgun Gothic'   # Windows 한글 폰트 (없으면 자동으로 기본 폰트로 대체됨)
plt.rcParams['axes.unicode_minus'] = False        # 마이너스 기호 깨짐 방지

chart_labels = ["AR" if lbl == "0/1" else lbl for lbl in sweep_labels]

paper_nrmse_list = [34.76, 34.89, 35.14, 36.28, 41.09, 44.45, 46.11, 48.57, 49.21, 49.61, 50.07]
paper_gap_list = [15.04, 13.91, 13.42, 12.71, 11.88, 11.44, 11.38, 11.38, 11.36, 11.36, 11.36]

fig, ax1 = plt.subplots(figsize=(9, 5))
ax2 = ax1.twinx()

ax1.plot(chart_labels, sweep_nrmse, color="#F97316", marker="o", label="nRMSE (우리 결과)")
ax1.plot(chart_labels, paper_nrmse_list, color="#FDBA74", marker="o", linestyle="--", label="nRMSE (논문 Table3)")
ax2.plot(chart_labels, sweep_gap, color="#3B82F6", marker="s", label="gap (우리 결과)")
ax2.plot(chart_labels, paper_gap_list, color="#93C5FD", marker="s", linestyle="--", label="gap (논문 Table3)")
...
plt.title("Fig.3 원본(3항, 정규화X) - z03 블록18")
plt.tight_layout()
plt.savefig("fig3_ar_original.png", dpi=150)
print("저장 완료: fig3_ar_original.png")
plt.show()


# =====================================================================
# 12. Figure 5 재현 - (a) nRMSE, (b) optimality gap 을 벌금비용률(0~100%)
#     별로 그린다. W1=W2=1 고정 (원본 3항 profit function 버전)
#     AR(기본) 선은 W1=0 이라 학습이 벌금율과 무관 - 한 번만 학습하고,
#     평가(gap)만 벌금율별로 다시 계산한다. Proposed 선은 벌금율마다
#     처음부터 다시 학습한다.
# =====================================================================
import matplotlib.pyplot as plt
...
ar_coefficients_by_hour_f5 = np.zeros((HOURS_PER_DAY, n_features))
for hour in range(HOURS_PER_DAY):
    y_this_hour = train_solar[:, hour]

    surplus_cost_f5 = np.full(n_train_obs, 1.0)
    shortage_cost_f5 = np.full(n_train_obs, 1.0)

    gmodel_f5 = gp.Model(f"ar_baseline_fig5_{hour}")
    gmodel_f5.Params.OutputFlag = 0
    gmodel_f5.Params.MIPGap = 1e-9

    beta_var_f5 = gmodel_f5.addMVar(n_features, lb=-GRB.INFINITY, name="beta")
    x_var_f5 = gmodel_f5.addMVar(n_train_obs, lb=0.0, ub=1.0, name="x")
    yplus_var_f5 = gmodel_f5.addMVar(n_train_obs, lb=0.0, ub=1.0, name="y_plus")
    yminus_var_f5 = gmodel_f5.addMVar(n_train_obs, lb=0.0, ub=1.0, name="y_minus")

    gmodel_f5.addConstr(x_var_f5 - ar_design_matrix @ beta_var_f5 == 0.0)
    gmodel_f5.addConstr(x_var_f5 + yplus_var_f5 - yminus_var_f5 == y_this_hour)

    objective_expr_f5 = surplus_cost_f5 @ yplus_var_f5 + shortage_cost_f5 @ yminus_var_f5
    gmodel_f5.setObjective(objective_expr_f5, GRB.MINIMIZE)
    gmodel_f5.optimize()

    ar_coefficients_by_hour_f5[hour] = beta_var_f5.X

ar_forecast_f5 = np.zeros((n_test_days, HOURS_PER_DAY))
previous_day_actual_f5 = train_solar[-1]
for day_index in range(n_test_days):
    feature_vector_f5 = np.concatenate([[1.0], previous_day_actual_f5[::-1]])
    for hour in range(HOURS_PER_DAY):
        raw_prediction = np.dot(ar_coefficients_by_hour_f5[hour], feature_vector_f5)
        ar_forecast_f5[day_index, hour] = min(max(raw_prediction, 0.0), 1.0)
    previous_day_actual_f5 = test_solar[day_index]

actual_flat_f5 = test_solar.flatten()
ar_predicted_flat_f5 = ar_forecast_f5.flatten()
da_flat_f5 = test_da_price.flatten()
rt_flat_f5 = test_rt_price.flatten()

sse_ar_f5 = 0.0
for i in range(len(actual_flat_f5)):
    e = actual_flat_f5[i] - ar_predicted_flat_f5[i]
    sse_ar_f5 = sse_ar_f5 + e * e
avg_actual_f5 = 0.0
for i in range(len(actual_flat_f5)):
    avg_actual_f5 = avg_actual_f5 + actual_flat_f5[i]
avg_actual_f5 = avg_actual_f5 / len(actual_flat_f5)
ar_nrmse_f5 = 100.0 * ((sse_ar_f5 / len(actual_flat_f5)) ** 0.5) / avg_actual_f5
...
for penalty_rate_sw in PENALTY_LIST:
    ...
    sum_oracle_ar = 0.0
    for i in range(len(actual_flat_f5)):
        actual_i = actual_flat_f5[i]
        commitment_i = ar_predicted_flat_f5[i]
        da_i = da_flat_f5[i]
        rt_i = rt_flat_f5[i]
    ...
    gap_ar_sw = 100.0 * (sum_oracle_ar - sum_realized_ar) / sum_oracle_ar
    ar_nrmse_list.append(ar_nrmse_f5)
    ar_gap_list.append(gap_ar_sw)

    coefficients_by_hour_p5 = np.zeros((HOURS_PER_DAY, n_features))
    for hour in range(HOURS_PER_DAY):
        y_this_hour = train_solar[:, hour]
        da_this_hour = train_da_price[:, hour]
        rt_this_hour = train_rt_price[:, hour]

        surplus_cost_p5 = np.zeros(n_train_obs)
        shortage_cost_p5 = np.zeros(n_train_obs)
        for i in range(n_train_obs):
            penalty_i = penalty_rate_sw * da_this_hour[i]
            surplus_cost_p5[i] = (-FIG5_W1 * rt_this_hour[i]) + FIG5_W2
            shortage_cost_p5[i] = (FIG5_W1 * penalty_i) + FIG5_W2

        binary_row_list_p5 = []
        for i in range(n_train_obs):
            if surplus_cost_p5[i] + shortage_cost_p5[i] < 0.0:
                binary_row_list_p5.append(i)
        binary_rows_p5 = np.array(binary_row_list_p5, dtype=int)
        n_binary_p5 = len(binary_rows_p5)

        gmodel_p5 = gp.Model(f"proposed_ar_fig5_{penalty_rate_sw}_{hour}")
        gmodel_p5.Params.OutputFlag = 0
        gmodel_p5.Params.MIPGap = 1e-9
        gmodel_p5.Params.TimeLimit = 60

        beta_var_p5 = gmodel_p5.addMVar(n_features, lb=-GRB.INFINITY, name="beta")
        x_var_p5 = gmodel_p5.addMVar(n_train_obs, lb=0.0, ub=1.0, name="x")
        yplus_var_p5 = gmodel_p5.addMVar(n_train_obs, lb=0.0, ub=1.0, name="y_plus")
        yminus_var_p5 = gmodel_p5.addMVar(n_train_obs, lb=0.0, ub=1.0, name="y_minus")

        gmodel_p5.addConstr(x_var_p5 - ar_design_matrix @ beta_var_p5 == 0.0)
        gmodel_p5.addConstr(x_var_p5 + yplus_var_p5 - yminus_var_p5 == y_this_hour)

        if n_binary_p5 > 0:
            z_var_p5 = gmodel_p5.addMVar(n_binary_p5, vtype=GRB.BINARY, name="z")
            gmodel_p5.addConstr(yplus_var_p5[binary_rows_p5] + z_var_p5 <= 1.0)
            gmodel_p5.addConstr(yminus_var_p5[binary_rows_p5] - z_var_p5 <= 0.0)

        objective_expr_p5 = (
            (-FIG5_W1 * da_this_hour) @ x_var_p5
            + surplus_cost_p5 @ yplus_var_p5
            + shortage_cost_p5 @ yminus_var_p5
        )
        gmodel_p5.setObjective(objective_expr_p5, GRB.MINIMIZE)
        gmodel_p5.optimize()

        coefficients_by_hour_p5[hour] = beta_var_p5.X

    proposed_forecast_p5 = np.zeros((n_test_days, HOURS_PER_DAY))
    previous_day_actual_p5 = train_solar[-1]
    for day_index in range(n_test_days):
        feature_vector_p5 = np.concatenate([[1.0], previous_day_actual_p5[::-1]])
        for hour in range(HOURS_PER_DAY):
            raw_prediction = np.dot(coefficients_by_hour_p5[hour], feature_vector_p5)
            proposed_forecast_p5[day_index, hour] = min(max(raw_prediction, 0.0), 1.0)
        previous_day_actual_p5 = test_solar[day_index]

    proposed_predicted_flat_p5 = proposed_forecast_p5.flatten()

    sse_p5 = 0.0
    for i in range(len(actual_flat_f5)):
        e = actual_flat_f5[i] - proposed_predicted_flat_p5[i]
        sse_p5 = sse_p5 + e * e
    nrmse_p5 = 100.0 * ((sse_p5 / len(actual_flat_f5)) ** 0.5) / avg_actual_f5

    sum_realized_p5 = 0.0
    sum_oracle_p5 = 0.0
    for i in range(len(actual_flat_f5)):
        actual_i = actual_flat_f5[i]
        commitment_i = proposed_predicted_flat_p5[i]
        da_i = da_flat_f5[i]
        rt_i = rt_flat_f5[i]
        penalty_cost_i = penalty_rate_sw * da_i
        mismatch_i = actual_i - commitment_i
        ...
        sum_realized_p5 = sum_realized_p5 + realized_profit_i
        ...
        oracle_profit_i = max(profit_if_commit_zero, profit_if_commit_actual, profit_if_commit_full)
        sum_oracle_p5 = sum_oracle_p5 + oracle_profit_i

    gap_p5 = 100.0 * (sum_oracle_p5 - sum_realized_p5) / sum_oracle_p5

    proposed_nrmse_list.append(nrmse_p5)
    proposed_gap_list.append(gap_p5)
    print(f"[벌금율 {penalty_rate_sw*100:.0f}%] AR nRMSE={ar_nrmse_f5:.4f}% gap={gap_ar_sw:.4f}% | Proposed nRMSE={nrmse_p5:.4f}% gap={gap_p5:.4f}%")

penalty_pct_labels = [f"{int(p*100)}%" for p in PENALTY_LIST]

# ---- 논문 Fig.5 그래프 이미지에서 읽은 값 (50%는 Table3 정확값으로 앵커링, 나머지는 추정치) ----
paper_ar_nrmse_list = [34.76] * 11   # AR nRMSE는 벌금율에 무관하게 상수 (Table3 값 그대로, 정확함)
paper_ar_gap_list = [9.0, 10.5, 12.0, 12.5, 14.0, 15.04, 16.5, 18.0, 19.3, 20.5, 21.7]   # 50%=15.04는 Table3 정확값, 나머지는 Fig.5(b) 이미지 추정치
paper_proposed_nrmse_list = [46, 34, 35, 38, 42, 44.45, 51, 56, 61, 64, 67]   # 50%=44.45는 Table3 정확값, 나머지는 Fig.5(a) 이미지 추정치
paper_proposed_gap_list = [8.5, 10.5, 11.5, 11.7, 11.5, 11.44, 11.5, 11.7, 11.7, 11.8, 12.0]   # 50%=11.44는 Table3 정확값, 나머지는 Fig.5(b) 이미지 추정치

penalty_pct_labels = [f"{int(p*100)}%" for p in PENALTY_LIST]

fig, (ax_a, ax_b) = plt.subplots(1, 2, figsize=(12, 5))
ax_a.plot(penalty_pct_labels, ar_nrmse_list, color="#3B82F6", marker="o", label="AR (우리 결과)")
ax_a.plot(penalty_pct_labels, paper_ar_nrmse_list, color="#93C5FD", linestyle="--", label="AR (논문 Fig.5 추정치)")
ax_a.plot(penalty_pct_labels, proposed_nrmse_list, color="#F97316", marker="s", label="Proposed (우리 결과)")
ax_a.plot(penalty_pct_labels, paper_proposed_nrmse_list, color="#FDBA74", linestyle="--", label="Proposed (논문 Fig.5 추정치)")
ax_a.set_xlabel("Penalty Cost Rate")
ax_a.set_ylabel("nRMSE (%)")
ax_a.set_title("(a) nRMSE")
ax_a.legend(fontsize=8)

ax_b.plot(penalty_pct_labels, ar_gap_list, color="#3B82F6", marker="o", label="AR (우리 결과)")
ax_b.plot(penalty_pct_labels, paper_ar_gap_list, color="#93C5FD", linestyle="--", label="AR (논문 Fig.5 추정치)")
ax_b.plot(penalty_pct_labels, proposed_gap_list, color="#F97316", marker="s", label="Proposed (우리 결과)")
ax_b.plot(penalty_pct_labels, paper_proposed_gap_list, color="#FDBA74", linestyle="--", label="Proposed (논문 Fig.5 추정치)")
ax_b.set_xlabel("Penalty Cost Rate")
ax_b.set_ylabel("Optimality Gap (%)")
ax_b.set_title("(b) Optimality Gap")
ax_b.legend(fontsize=8)

plt.suptitle("Fig.5 원본(3항, 정규화X) - z03 블록18 (W1=W2=1)")
plt.tight_layout()
plt.savefig("fig5_ar_original.png", dpi=150)
print("저장 완료: fig5_ar_original.png")
plt.show()