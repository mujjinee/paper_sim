...
for hour in range(HOURS_PER_DAY):                       # 시간대 0부터 11까지 순서대로 하나씩 처리
    ...
    for i in range(n_train_obs):                                 # 300개 학습 표본을 하나씩 확인
        ...
        profit_commit_1 = CAPACITY_MW * DURATION_HOURS * (
            da_i * 1.0 + rt_i * surplus_if_full - (penalty_i + rt_i) * shortage_if_full   # [수정] 4항
        )                                                             # ↑ 약정 = 설비최대(1.0)일 때 이익
    ...
    for i in range(n_train_obs):                                    # 300개를 하나씩 순서대로
        ...
        surplus_cost[i] = (-W1 * rt_this_hour[i]) + W2   # [수정] 정규화 제거
        shortage_cost[i] = (W1 * (penalty_i + rt_this_hour[i])) + W2   # [수정] 정규화 제거, 4항(부족비용=벌금+RT가격)은 유지
...
# =====================================================================
# 10. Table 3 스윕 - 0/1(기존 재현) + W1/W2 비율 10개 (4항 profit_change)
# =====================================================================
...
for ratio_label, sweep_w1, sweep_w2 in RATIO_LIST:
    ...
    for hour in range(HOURS_PER_DAY):
        ...
        for i in range(n_train_obs):
            ...
            shortage_cost_sw[i] = (sweep_w1 * (penalty_i + rt_this_hour[i])) + sweep_w2
...
plt.title("Fig.3 4항(profit_change, 정규화X) - z03 블록18")
plt.tight_layout()
plt.savefig("fig3_ar_profit_change.png", dpi=150)
print("저장 완료: fig3_ar_profit_change.png")
plt.show()
...
for penalty_rate_sw in PENALTY_LIST:
    ...
    for hour in range(HOURS_PER_DAY):
        ...
        for i in range(n_train_obs):
            ...
            shortage_cost_p5[i] = (FIG5_W1 * (penalty_i + rt_this_hour[i])) + FIG5_W2
...
paper_ar_nrmse_list = [34.76] * 11
paper_ar_gap_list = [9.0, 10.5, 12.0, 12.5, 14.0, 15.04, 16.5, 18.0, 19.3, 20.5, 21.7]
paper_proposed_nrmse_list = [46, 34, 35, 38, 42, 44.45, 51, 56, 61, 64, 67]
paper_proposed_gap_list = [8.5, 10.5, 11.5, 11.7, 11.5, 11.44, 11.5, 11.7, 11.7, 11.8, 12.0]
...
plt.suptitle("Fig.5 4항(profit_change, 정규화X) - z03 블록18 (W1=W2=1)")
plt.tight_layout()
plt.savefig("fig5_ar_profit_change.png", dpi=150)
print("저장 완료: fig5_ar_profit_change.png")
plt.show()