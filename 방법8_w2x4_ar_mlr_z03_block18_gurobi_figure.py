# -*- coding: utf-8 -*-
# =====================================================================
# 방법8_w2x4_ar_mlr_z03_block18_gurobi_figure.py
#
# `방법8_w2x4_ar_mlr_z03_block18_gurobi.py`(검증 완료된 Gurobi 변환본)를 그대로
# 복사한 뒤, savefig/CSV 저장 경로에만 "_gurobi" 접미사를 붙인 버전이다.
# Figure 생성 코드 자체는 새로 짠 것이 아니라 원본 `방법2_4term_ar_mlr_z03_
# block18.py`(scipy) 안에 원래 있던 코드를 그대로 물려받은 것이다 — 이 파일이
# 곧 results/방법2_fig{3,5,6,8}_..._z03_block18.png를 실제로 만드는 스크립트였다
...
#
# PC = RT + rate×DA (DA*x + RT*surplus - PC*shortage, 3항 구조)
# AR(시간대별 12개 모델) + MLR(pooled 1개 모델) 통합 실행
# 대상: z03/블록18 (2013년 가을)
#
# 출력:
#   - 전체 그리드(50조합) + Fig3/5/6/8 스윕
#   - 논문 KPI 조건(W1=1, W2=20, rate=50%) 별도 print
...
# ── W2 균형 보정 Weight (오늘_GapRate비교.md §10~11 디버깅 결론) ──
# 원본 Gurobi 코드는 목적함수를 scale/denom/n_obs로 정규화하지 않는 raw 계수를 쓴다
# (obj_x=-W1*DA, sc=-W1*RT+W2, yc=W1*PC+W2, PC=RT+rate*DA). 이 raw 계수 그대로 쓰면
# W1 항 크기(≈가격 수준 20~30) 가 W2(=1) 보다 20~30배 커서 economic 항만 지배해버리므로
# (nRMSE 59~73%까지 치솟음), W2에 Weight를 곱해 W1과 같은 자릿수로 맞춰준다: W2_eff = W2 * WEIGHT.
# temp_debug_fig5_weight_sweep_v2/v3.py 로 Weight 1~15를 스윕해 논문 Fig.5(AR, rate 0~100%)
# 판독값과 SSE 비교한 결과 Weight=4가 최적(SSE=369, 다음 3=380, 5=430) — KPI 기준점(1/20, 1/10, 1/1)
# 에서도 논문과 nRMSE ±0.3~1.5%p로 근접 확인됨. (Weight=15는 과도하게 강해 SSE=1220으로 더 나쁨)
W2_BALANCE_WEIGHT = 4

# ── 논문 참고값 (자신과 비교 — Table 3) ──
...
# 논문 Table 4 (MLR 제안모형, W1/W2 10개점) — AR(Table 3)과 다른 값이므로 별도로 둠
...
# PyMuPDF로 페이지를 600dpi로 렌더링 후, 곡선 색상(RGB) 픽셀 좌표를 축 눈금 기준으로
# 데이터값으로 역산 (범례 상자는 bounding box로 제외). 50% 지점이 Table 4 실측치
# (MLR 21.76%/12.59%, 제안모형 W1=W2=1: 22.01%/10.28%)와 정확히 일치해 추출 신뢰도 확인됨.
...
def auto_ylim(*series, pad_frac=0.12, round_to=5, floor_at_zero=True):
    ...
    여러 시리즈(논문값 + 재현값)를 합쳐서 min/max를 구하고, 위아래로 pad_frac만큼 여유를
    두고 round_to 단위로 반올림한다."""
    vals = []
    for s in series:
...
def solve_ar_proposed(penalty_rate, W1, W2):
    """AR 제안모형: 12시간대별 MILP 풀기 (raw 계수 + W2 balance weight)"""
...
# =====================================================================
# 7. 제안모형 MILP — MLR (pooled)
# =====================================================================
def solve_mlr_proposed(penalty_rate, W1, W2):
    """MLR 제안모형: pooled MILP 하나만 풀기 (raw 계수 + W2 balance weight)"""
    W2_eff = W2 * W2_BALANCE_WEIGHT
    n_obs = n_train_obs
...
def get_ar(pr, w1, w2):
    ...
    if k not in cache_ar:
        ...
        cache_ar[k] = (nrmse(pred, actual_flat_all),
...
def get_mlr(pr, w1, w2):
    ...
    if k not in cache_mlr:
        ...
        cache_mlr[k] = (nrmse(pred, actual_flat),
                        compute_gap_4term(pred, pr, actual_flat, da_flat, rt_flat))
    return cache_mlr[k]


# =====================================================================
# 9. 전체 그리드 스윕 (50조합) — AR + MLR 병렬
# =====================================================================
print("\n" + "=" * 70)
...
# 그리드 CSV
grid_csv = os.path.join(OUT_DIR, "grid_4term_ar_mlr_z03_block18_gurobi.csv")
...
ar_kpi_gap = compute_gap_4term(ar_pred_flat, KPI_RATE,
                                actual_flat_all, da_flat_all, rt_flat_all)
mlr_kpi_gap = compute_gap_4term(mlr_pred_flat, KPI_RATE,
...
print(f"""
  ...
  ├──────────────────┼──────────────┼──────────────┼──────────────┼──────────────┤
  │  AR (z03/블록18)  │ {ar_kpi_n:>10.2f}   │ {ar_kpi_g:>10.2f}   │ {ar_nrmse:>10.2f}   │ {ar_kpi_gap:>10.2f}   │
  │  MLR (z03/블록18) │ {mlr_kpi_n:>10.2f}   │ {mlr_kpi_g:>10.2f}   │ {mlr_nrmse:>10.2f}   │ {mlr_kpi_gap:>10.2f}   │
  └──────────────────┴──────────────┴──────────────┴──────────────┴──────────────┘
...
# =====================================================================
# 11. Best rate 자동탐색 (그리드 기준)
# =====================================================================
# AR: baseline nRMSE 차이 + gap 차이 최소화 (1/20 조합 기준)
best_ar_err = np.inf; best_ar_rate = None
best_mlr_err = np.inf; best_mlr_rate = None
for pr, W1, W2, a_n, a_g, m_n, m_g in grid_results:
    if (W1, W2) == (1, 20):
        ar_err = abs(a_n - ar_nrmse) + abs(a_g - ar_kpi_gap)
        mlr_err = abs(m_n - mlr_nrmse) + abs(m_g - mlr_kpi_gap)
        if ar_err < best_ar_err:
            best_ar_err = ar_err; best_ar_rate = pr
        if mlr_err < best_mlr_err:
            best_mlr_err = mlr_err; best_mlr_rate = pr

# Fig.3/6은 grid에서 가장 좋은 rate, Fig.5/8은 rate sweep
# 편의상 AR/MLR 모두 rate=0.5 (논문 KPI rate)에서 Fig.3/6을 그킴
FIG36_RATE = KPI_RATE  # 0.5
print(f"\n  Fig.3/6 고정 rate: {FIG36_RATE} (논문 KPI)")


# =====================================================================
# 12. Fig.3 (AR, W1/W2 10개점, rate 고정) + Fig.6 (MLR 동일) 데이터
# =====================================================================
print("\n" + "=" * 70)
print(f"  6. Fig.3/6 데이터 — W1/W2 10개점 스윕 (rate={FIG36_RATE})")
print("=" * 70)

fig3_ar_n = [ar_nrmse]; fig3_ar_g = [ar_kpi_gap if FIG36_RATE == KPI_RATE
                                      else compute_gap_4term(ar_pred_flat, FIG36_RATE,
                                      actual_flat_all, da_flat_all, rt_flat_all)]
fig6_mlr_n = [mlr_nrmse]; fig6_mlr_g = [mlr_kpi_gap if FIG36_RATE == KPI_RATE
                                         else compute_gap_4term(mlr_pred_flat, FIG36_RATE,
                                         actual_flat, da_flat, rt_flat)]
...
# =====================================================================
# 13. Fig.5 (AR, rate 0~150%, W1=W2=1) + Fig.8 (MLR 동일) 데이터
# =====================================================================
print("\n" + "=" * 70)
print(f"  7. Fig.5/8 데이터 — rate 0~100% 스윕 (W1={FIG58_W1}, W2={FIG58_W2}, W2_eff=W2×{W2_BALANCE_WEIGHT} 균형 보정)")
print("=" * 70)
...
for rate in FIG_RATES:
    ar_g_r = compute_gap_4term(ar_pred_flat, rate, actual_flat_all, da_flat_all, rt_flat_all)
    mlr_g_r = compute_gap_4term(mlr_pred_flat, rate, actual_flat, da_flat, rt_flat)
    ...
    a_n, a_g = get_ar(rate, FIG58_W1, FIG58_W2)
    m_n, m_g = get_mlr(rate, FIG58_W1, FIG58_W2)
...
save_csv(os.path.join(OUT_DIR, "fig3_ar_z03_block18_gurobi.csv"),
         ["Label", "nRMSE", "Gap"],
         [[FIG_LABELS[i], round(fig3_ar_n[i], 2), round(fig3_ar_g[i], 2)]
          for i in range(len(FIG_LABELS))])

save_csv(os.path.join(OUT_DIR, "fig5_ar_z03_block18_gurobi.csv"),
...
save_csv(os.path.join(OUT_DIR, "fig6_mlr_z03_block18_gurobi.csv"),
         ["Label", "nRMSE", "Gap"],
         [[FIG_LABELS[i], round(fig6_mlr_n[i], 2), round(fig6_mlr_g[i], 2)]
          for i in range(len(FIG_LABELS))])

save_csv(os.path.join(OUT_DIR, "fig8_mlr_z03_block18_gurobi.csv"),
...
fig.suptitle(f"방법2_fig3 — AR PC=RT+rate×DA, rate={FIG36_RATE}, W1/W2 스윕 (dual y축)", fontsize=13)
ax_r = ax_l.twinx()

paper_nrmse_all = [PAPER_AR_NRMSE] + PAPER_PROP_NRMSE
paper_gap_all = [PAPER_AR_GAP] + PAPER_PROP_GAP
ln1 = ax_l.plot(x, paper_nrmse_all, marker="o", color="#4C72B0",
                linewidth=2, linestyle="--", label="논문 nRMSE")
ln2 = ax_l.plot(x, fig3_ar_n,   marker="o", color="#1a3a6b",
                linewidth=2, linestyle="-", label="재현 nRMSE")
ln3 = ax_r.plot(x, paper_gap_all, marker="s", color="#eb9834",
                linewidth=2, linestyle="--", label="논문 Gap")
ln4 = ax_r.plot(x, fig3_ar_g,   marker="s", color="#b35900",
...
fig.tight_layout()
p3 = os.path.join(RESULTS_DIR, "방법2_fig3_ar_z03_block18_gurobi.png")
fig.savefig(p3, dpi=150); plt.close(fig)
print(f"  saved: {p3}")


# =====================================================================
# 16. Fig.5 — AR, rate 스윕 (nRMSE / Gap 2분할)
# =====================================================================
print("\n" + "=" * 70)
...
fig5_fig.suptitle(f"방법2_fig5 — AR PC=RT+rate×DA, W1={FIG58_W1}/W2={FIG58_W2}(W2_eff=W2×{W2_BALANCE_WEIGHT} 균형 보정), penalty rate 스윕", fontsize=13)
...
ax5n.plot(x5, fig5_ar_n_list, marker="o", color="#1a3a6b", linewidth=2, linestyle="-", label="재현 AR")
ax5n.plot(x5, fig5_ar_prop_n, marker="s", color="#b35900", linewidth=2, linestyle="-", label="재현 제안모형")
...
ax5g.plot(x5_paper, FIG5_PAPER_PROP_GAP, linestyle="--", color="#eb9834", alpha=0.8,
          marker="s", markersize=4, label="논문 제안모형")
ax5g.plot(x5, fig5_ar_g_list, marker="o", color="#1a3a6b", linewidth=2, linestyle="-", label="재현 AR")
ax5g.plot(x5, fig5_ar_prop_g, marker="s", color="#b35900", linewidth=2, linestyle="-", label="재현 제안모형")
...
p5 = os.path.join(RESULTS_DIR, "방법2_fig5_ar_z03_block18_gurobi.png")
...
fig6_fig.suptitle(f"방법2_fig6 — MLR PC=RT+rate×DA, rate={FIG36_RATE}, W1/W2 스윕 (dual y축)", fontsize=13)
ax_r = ax_l.twinx()

paper_mlr_nrmse_all = [PAPER_MLR_NRMSE] + PAPER_PROP_MLR_NRMSE
paper_mlr_gap_all = [PAPER_MLR_GAP] + PAPER_PROP_MLR_GAP
ln1 = ax_l.plot(x, paper_mlr_nrmse_all, marker="o", color="#4C72B0",
                linewidth=2, linestyle="--", label="논문 nRMSE")
ln2 = ax_l.plot(x, fig6_mlr_n,  marker="o", color="#1a3a6b",
                linewidth=2, linestyle="-", label="재현 nRMSE")
ln3 = ax_r.plot(x, paper_mlr_gap_all, marker="s", color="#eb9834",
                linewidth=2, linestyle="--", label="논문 Gap")
ln4 = ax_r.plot(x, fig6_mlr_g,  marker="s", color="#b35900",
                linewidth=2, linestyle="-", label="재현 Gap")
...
p6 = os.path.join(RESULTS_DIR, "방법2_fig6_mlr_z03_block18_gurobi.png")
fig6_fig.savefig(p6, dpi=150); plt.close(fig6_fig)
print(f"  saved: {p6}")


# =====================================================================
# 18. Fig.8 — MLR, rate 스윕 (nRMSE / Gap 2분할)
# =====================================================================
print("\n" + "=" * 70)
...
fig8_fig.suptitle(f"방법2_fig8 — MLR PC=RT+rate×DA, W1={FIG58_W1}/W2={FIG58_W2}(W2_eff=W2×{W2_BALANCE_WEIGHT} 균형 보정), penalty rate 스윕", fontsize=13)
...
ax8n.plot(x5_paper, FIG8_PAPER_PROP_NRMSE, linestyle="--", color="#8CAED6", alpha=0.8,
          marker="s", markersize=4, label="논문 제안모형")
ax8n.plot(x5, fig8_mlr_n_list, marker="o", color="#7a1f28", linewidth=2, linestyle="-", label="재현 MLR")
ax8n.plot(x5, fig8_mlr_prop_n, marker="s", color="#3d6a8f", linewidth=2, linestyle="-", label="재현 제안모형")
...
ax8g.plot(x5_paper, FIG8_PAPER_PROP_GAP, linestyle="--", color="#8CAED6", alpha=0.8,
          marker="s", markersize=4, label="논문 제안모형")
ax8g.plot(x5, fig8_mlr_g_list, marker="o", color="#7a1f28", linewidth=2, linestyle="-", label="재현 MLR")
ax8g.plot(x5, fig8_mlr_prop_g, marker="s", color="#3d6a8f", linewidth=2, linestyle="-", label="재현 제안모형")
if hl is not None:
...
p8 = os.path.join(RESULTS_DIR, "방법2_fig8_mlr_z03_block18_gurobi.png")
fig8_fig.savefig(p8, dpi=150); plt.close(fig8_fig)
print(f"  saved: {p8}")

# =====================================================================
print("\n" + "=" * 70)
print("  완료 — AR+MLR PC=RT+rate×DA 통합 실험 (z03/블록18)")
print("=" * 70)
print(f"""
  생성 파일:
    Fig.3 (AR W1/W2):   {p3}
