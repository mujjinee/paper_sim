# -*- coding: utf-8 -*-
"""
AR(자기회귀) 모델을 120일 단위 블록(90일 학습 + 30일 테스트)으로 나눠 돌리는 스크립트.

ar_only_full_range.py 와 로직은 완전히 동일하고, 블록 크기와 학습/테스트 일수만 다르다:
  - ar_only_full_range.py : 400일 블록 (학습 300일 + 테스트 100일) - 논문과 동일한 크기
  - 이 스크립트           : 120일 블록 (학습  90일 + 테스트  30일) - 더 잘게 쪼갠 버전

블록을 잘게 쪼개면 predictors15.csv 전체 820일 구간(2012-04~2014-06)을
6개 블록으로 촘촘히 나눠볼 수 있어서, "계절에 따라 AR 성능이 얼마나 달라지는지"를
더 세밀하게 확인할 수 있다.

실행 방법:
    python ar_only_120day_blocks.py
"""
...
TRAIN_DAYS = 90              # 학습(모델을 만드는 데 쓰는) 기간의 날짜 수  <- 이번엔 90일
TEST_DAYS = 30               # 테스트(성능을 확인하는 데 쓰는) 기간의 날짜 수  <- 이번엔 30일
WINDOW_DAYS = TRAIN_DAYS + TEST_DAYS   # 학습+테스트를 합친 한 구간의 길이 = 120일
...
# ============================================================
# 4. 한 구간(120일)에 대해 학습 + 테스트 + 채점을 실행
# ============================================================
def run_ar_on_one_window(zone_label, day_list, power_table, window_start_index):
    """
    전체 날짜 중 window_start_index번째 날짜부터 120일(학습 90 + 테스트 30)을 잘라내서,
    그 구간으로 AR 모델을 학습시키고 테스트해서 nRMSE를 계산해준다.
    """
    window_end_index = window_start_index + WINDOW_DAYS              # 이 구간이 끝나는 위치 (시작 + 120)
    ...
    train_power = window_power[:TRAIN_DAYS]                # 이 구간의 앞쪽 90일 = 학습용 발전량 표
    test_power = window_power[TRAIN_DAYS:]                  # 이 구간의 뒤쪽 30일 = 테스트용 발전량 표
    ...
    # ---- 테스트 30일을, 실제로 서비스하듯 "하루씩 순서대로" 예측한다 ----
    ...
    for test_day_index in range(TEST_DAYS):                    # 테스트 30일을 0번째부터 29번째까지 순서대로
    ...
    # ---- 예측이 얼마나 정확했는지 채점 ----
    actual_flat = test_power.flatten()                # (30, 12) 표를 360개짜리 한 줄로 펼침
...
def main():
    ...
    for zone_id in ZONE_ID_LIST:                    # zone 1, 2, 3을 순서대로 하나씩 처리
        ...
        number_of_blocks = total_days // WINDOW_DAYS         # 120일짜리 구간을 몇 개나 만들 수 있는지 계산 (나머지 버림)
        ...
        leftover_days = total_days - number_of_blocks * WINDOW_DAYS   # 120일 구간으로 못 나눈 나머지 날짜 수
        if leftover_days > 0:                                          # 나머지가 있다면
            print(f"[{zone_label}] 마지막 {leftover_days}일은 120일이 안 돼서 사용하지 않음")  # 안내 메시지 출력

    # ---- 모든 결과를 표로 정리해서 마지막에 한 번 더 보여줌 ----
    print("\n=== 전체 결과 요약 (120일 블록 = 90일 학습 + 30일 테스트) ===")   # 요약 제목 출력
    ...
    print(f"\n논문 AR nRMSE = {PAPER_NRMSE:.2f}%  (비교 기준값, 단 논문은 300일/100일 실험이라 완전히 동일 조건은 아님)")
