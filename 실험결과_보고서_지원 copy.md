...
이후 방법1 profit, 방법2 profit, 방법3 profit, Smart SPO+ 및 CVaR의 대표 사례 비교에는 같은 z03 block 18을 사용하였다. 한편 CVaR의 적용 결과를 확인하기 위해 별도로 수행한 72개 블록 검증은 Proposed AR과 방법2 profit을 사용한 후속 실험으로, 이 절의 대표 블록 선정 과정과는 구분된다.
...
방법 6은 profit function 대신 학습 loss를 Smart SPO+ 형태로 변경한 실험이다. 일부 조건에서는 논문의 KPI 및 곡선과 가까운 결과가 나왔지만, 원본 profit function의 구조 자체를 바꾼 것은 아니므로 profit 정의에 따라 결과가 달라졌다.
...
논문 Table 3(AR)·Table 4(MLR)이 비교 기준으로 삼는 지점(W1=1, W2=20, 벌금율 50%)에서, "AR+MLR 통합, W1/W2·penalty rate 스윕, Fig.3/5/6/8을 한 파일에서 바로 생성"하는 스크립트로 여섯 방법을 나란히 놓고 논문과의 격차(Δ)를 정리했다. 데이터셋은 2.4절 대표 사례(z03/블록18)로 일곱 방법(1~5, 6.1, 6.2) 전부 통일했다.<span style="color:red; text-decoration:line-through"> — 방법6.1·6.2는 원래 z01/블록21로 따로 검증했었으나, 다른 방법들과 직접 비교하기 위해 z03/블록18로 다시 돌렸다(5.6절).</span>
...
방법 6은 학습 loss를 Smart SPO+ 형태로 변경한 실험이다. 일부 조건에서는 논문의 KPI와 가까운 결과가 나왔지만, profit function의 구조에 따른 영향은 여전히 남았다. 이를 통해 학습 loss의 변경만으로 원본 3항식에서 나타난 현상을 일관되게 해결하기는 어렵다는 점을 확인하였다.
...
\* <span style="color:red; text-decoration:line-through">표 2(3.3.2절)에는 없는 값이다 — `integrated_paper_base_AR_MLR.py`의 방법1 2세대 MLR 결과(nRMSE 24.25%, Gap 3.02%, 논문 대비 ΔnRMSE +2.33%p·ΔGap −8.89%p)를 따로 계산해 반영했다.</span> <span style="color:blue">표 2(3.3.2절)에는 없는 값이다 — 방법1의 MLR 결과(nRMSE 24.25%, Gap 3.02%, 논문 대비 ΔnRMSE +2.33%p·ΔGap −8.89%p)를 따로 계산해 반영했다.</span>
